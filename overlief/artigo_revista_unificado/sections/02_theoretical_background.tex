...
and $P$ admissible variables available at forecast origin $t$. A deterministic
multi-output forecaster is a mapping
\begin{equation}
  \widehat{\mathbf{y}}_{\ell,t+1:t+H}
  =
...
$C\Delta$ is the physical context span, and $H\Delta$ is the physical forecast
horizon. Thus, 72 hourly outputs and three daily outputs have the same nominal
...
(FFT) of sample $b$ at frequency bin $f$ and channel $c$. In the implemented
model, the spectrum used to select periods is averaged over both the batch and
the channels,
\begin{equation}
  \overline{A}_f
  =
  \frac{1}{B C_x}
  \sum_{b=1}^{B}\sum_{c=1}^{C_x}\left|F_{b,f,c}\right|.
  \label{eq:timesnet_frequency}
\end{equation}
The zero-frequency component is excluded, and the $k$ largest amplitudes
select frequency indices $f_i$ and associated integer periods
\begin{equation}
  \{f_1,\ldots,f_k\}
  =
...
\end{equation}
The selected period set is consequently shared by the samples in a batch.
The amplitude retained for aggregation remains sample-specific,
$a_{b,i}=C_x^{-1}\sum_c |F_{b,f_i,c}|$.

For each $p_i$, the sequence is zero-padded to
$L_i=p_i\lceil T/p_i\rceil$ and reshaped so that one axis indexes position
within a period and the other indexes successive periods. A two-dimensional
Inception-style convolution then learns intraperiod and interperiod
variations. If $\mathcal{G}_i$ is the processed tensor and
$\mathcal{R}_i^{-1}$ restores chronological order, the period-specific
...
\begin{equation}
  ...
  \sum_{i=1}^{k}
  \alpha_{b,i}
  \left[\mathcal{R}_i^{-1}(\mathcal{G}_{b,i})\right]_{1:T}.
...
within-period position and recurrence across periods. Because period selection
is batch-shared, batch composition and deterministic ordering form part of the
implementation contract, whereas the aggregation weights remain specific to
each sample.
...
matched experimentally before performance differences can be attributed to
the architectures.
