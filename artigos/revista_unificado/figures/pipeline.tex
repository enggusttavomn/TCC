...
\node[a step,text width=0.175\textwidth,right=2mm of sources] (audit) {
  {\bfseries Audit and aggregation}\\[0.8mm]
...
\node[a step,text width=0.165\textwidth,right=2mm of audit] (contract) {
  {\bfseries Matched information}\\[0.8mm]
...
\node[a step,text width=0.235\textwidth,right=2mm of contract] (tasks) {
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    Monthly exploratory & $12\,\mathrm{mo}\rightarrow6\,\mathrm{mo}$
  \end{tabular}
};
...
\node[b step,text width=0.175\textwidth,right=2mm of b-label] (models) {
  ...
  \textbf{Deterministic:} persistence, annual seasonal na\"ive, climatology\\[0.5mm]
...
\draw[flow] (selection.east) -- node[above,font=\scriptsize\bfseries,text=orange!82!black]
  {discard + reset} (refit.west);
...
\node[c step,text width=0.17\textwidth,right=2mm of c-label] (test) {
  {\bfseries Frozen 2024 test}\\[0.8mm]
...
\node[c step,text width=0.17\textwidth,right=2mm of test] (physics) {
  ...
  inverse scale $\rightarrow$ nonnegative floor\\
  hourly nighttime-zero rule\\
...
are audited and converted into four direct-output tasks using causal GHI and
site identity only. (B) Matched models follow leakage-controlled selection and
fresh pre-test refitting. (C) Frozen 2024 forecasts receive common physical
