...
The executable study defines four frozen tasks rather than comparing errors
across unlike resolutions. Table~\ref{tab:study_design} lists their contexts,
direct outputs, evaluation horizons, and expected origin counts. Within a task,
all models share sites, origins, targets, preprocessing cuts, and
post-processing. Across tasks, metrics retain their resolution-specific units
and are not pooled.
...
learned references (XGBoost, LSTM, TimesNet, and DilatedRNN). Every learned
...
refit windows for exactly that number of epochs. XGBoost uses its frozen tree
budget in separate adjustment and refit instances. Test forecasts are generated
only by the refitted instances.

Figure~\ref{fig:temporal_protocol} summarizes this selection--refit chronology
and the four direct-output contracts that remain fixed during testing.

\input{figures/temporal_protocol}
...
Complete-target constraints restrict origins to January--July: adjustment has
31 origins from January 2020 through July 2022, validation has seven in 2023,
refit has 43 through July 2023, and test has seven in 2024. This task is
explicitly labelled exploratory because each site contributes only seven test
origins and the output covers one half-year.
...
interpret the main comparison. Appendix~\ref{app:hyperparameters} records the
...
Following the capacity reporting used to interpret architecture comparisons,
